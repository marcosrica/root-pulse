...
const changeWateringTime = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (userId != -1) {
    ...
    if (id != -1) {
      ...
      if (editable) {
        const result = await db.changeWateringTime(id, req.body.newTime);
...
export default changeWateringTime;
